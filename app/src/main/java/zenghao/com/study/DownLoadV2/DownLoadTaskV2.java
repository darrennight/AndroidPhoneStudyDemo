 ...
 */
public class DownLoadTaskV2 extends IDownLoadTaskV2 {

    private IDownLoadListenerV2 mListener;
    private static final int DEFAULT_TIMEOUT = 20000;
    public DownLoadTaskV2(IDownLoadInfoV2 info,IDownLoadListenerV2 listener){
        super(info);
            ...
            try {
                ...
                if(code == 200 || code == 206){
                    ...
                    if(createFile(mInfo.getFilePath(),mInfo.getFileName())){
                        ...
                        if(code == 200){
                            //不支持断点
                            mInfo.setStatus(DownStatusV2.STARTING);
                            directDown(conn);
                        } else{
                            //支持断点
                            breakPointDown();
    ...
    private void DownLoadInit(HttpURLConnection conn){
        ...
        if (TextUtils.isEmpty(mInfo.getFileName()))
            mInfo.setFileName(
                    HttpUtilV2.obtainFileName(mInfo.getDownUrl(), mInfo.getDisposition(), mInfo.getLocation()));
    ...
    private void breakPointDown(){
        ...
        IHttpClientV2 client = new MyHttpClientV2();
    ...
    private void addRequestHeaders(HttpURLConnection conn) {
        for (DLHeaderV2 header : HttpUtilV2.initRequestHeaders(new ArrayList<DLHeaderV2>(),mInfo)) {
