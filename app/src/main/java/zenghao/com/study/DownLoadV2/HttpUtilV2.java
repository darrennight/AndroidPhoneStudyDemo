...
public class HttpUtilV2 {
     ...
     * @return
     */
    public static List<DLHeaderV2> initRequestHeaders(List<DLHeaderV2> headers, IDownLoadInfoV2 info) {
        if (null == headers || headers.isEmpty()) {
            headers = new ArrayList<>();
            headers.add(new DLHeaderV2("Accept", "image/gif, image/jpeg, image/pjpeg, image/pjpeg," +
                    "application/x-shockwave-flash, application/xaml+xml," +
                    "application/vnd.ms-xpsdocument, application/x-ms-xbap," +
                    "application/x-ms-application, application/vnd.ms-excel," +
                    "application/vnd.ms-powerpoint, application/msword, */*"));
            headers.add(new DLHeaderV2("Accept-Ranges", "bytes"));
            headers.add(new DLHeaderV2("Charset", "UTF-8"));
            headers.add(new DLHeaderV2("Connection", "Keep-Alive"));
            headers.add(new DLHeaderV2("Accept-Encoding", "identity"));
            headers.add(new DLHeaderV2("Range", "bytes=" + 0 + "-"));
        }
        if (!hasRequestHeader("User-Agent", headers)) {
            headers.add(new DLHeaderV2("User-Agent", String.format("%s/%s (Linux; Android %s; %s Build/%s)", "MY_APP_NAME", "MY_APP_VERSION_NAME", Build.VERSION.RELEASE, Build.MANUFACTURER, Build.ID)));
        }
    ...
    private static boolean hasRequestHeader(String key, List<DLHeaderV2> headers) {
        for (DLHeaderV2 header : headers) {
